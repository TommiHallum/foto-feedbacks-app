...
def create_pdf(img, exif, intro, s1, s2, s3, s4):
    ...
    # Reduceret billede størrelse for printvenlighed
...
if uploaded:
    ...
    c1, c2 = st.columns([1.5, 1]) # Billedet er gjort mindre for at matche data
    with c1: st.image(img, use_container_width=True)
    with c2: 
        st.write("### Tekniske Data")
        if exif_data: st.table(list(exif_data.items())) # Viser data mere kompakt
